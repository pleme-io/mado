...
1. **World A retirement** (smallest): `spawn_term` spawns INTO the GUI's embedded `InProcess` via kanshou-forward (the `suggest_inject` idiom) instead of the process-local `SessionRegistry`; headless fallback keeps the old path. World A stops existing as a separate world for GUI-present setups; picker sees the sessions with zero picker changes (the reconciler already handles InProcess). Requires a `spawn`-capable kanshou leaf (today's leaves are read/inject only).
