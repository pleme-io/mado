...
| **M3** | ◐ SUPERSEDED by the izumi extraction (2026-07-05): the suggest plane's generic algebra (core/store/source/env/pace + livestream) now lives in [pleme-io/izumi](https://github.com/pleme-io/izumi) — mado consumes `izumi` + `izumi-sources` + `izumi-config` via git+rev (commit f80574c rewire; shim modules keep every import path + wire format byte-compatible, golden-fixture-proven). The `(defsuggestionsource)` authoring destination is izumi-lisp's `(defizumisource …)` (shipped there); the praça `SessionOrigin::Suggested` lift remains tear-side future work (gated on tear-daemon M5 + a cwd/env/args-bearing backend spawn op — see izumi/CLAUDE.md non-goals). | ◐ |
...
1. ✅ **World A retirement** (SHIPPED 2026-07-02, 17b2aed): `spawn_term` forwards to a new kanshou `spawn_term` leaf spawning into the GUI's embedded `InProcess` (tagged `SessionSource::Agent`); the reconciler absorbs it into praca — ● row + live-dedup anchor with zero picker changes. `TermSpec.world` selector (auto/embedded/headless; tests pin headless so a suite run can never spawn into the operator's GUI). Follow-ups: ✅ agent I/O into embedded panes (SHIPPED 834d25c — kanshou send_keys_embedded leaf; MCP send_keys forwards on local-miss ONLY for 16-hex tear-id shapes, so headless ids and test fakes stay local); ⏭ cwd-at-spawn (needs an atomic spawn-with-env tear-core API; the leaf refuses to race the picker's set_spawn_env→spawn two-step); ✅ get_output/snapshot for embedded panes (SHIPPED d3fdb38 — pane_snapshot_embedded leaf; get_output forwards by the same 16-hex id gate and honors lines by tailing). The embedded agent loop is COMPLETE: spawn → type → observe.
2. **World C rows** (medium): a daemon-session source feeding `praca.index` — either a picker-side `DaemonSessionReconciler` (a second reconciler against `Client::list_sessions` when `tear.runtime = daemon` or a daemon socket is discoverable) or upstream tear M2 (one shared PracaStore). Prefer riding tear's M2 per SESSION-TYPESCAPE — coordinate there before building mado-side. **Coordination read 2026-07-02:** tear M2 is a DEDICATED production-rewire arc (decide()→AttachAction + SessionRecord-as-projected-view + delete SessionState::Templated + ProjectBinding→InstanceRegistry + one-PracaStore, behavior-changing persisted-format migration staged with backward-compat deser — SESSION-TYPESCAPE §7). Phase 2 BLOCKS on it; schedule as its own arc, not tick-work.
