...
| **SuggestHealthJanitor** | `src/janitors.rs` | Files a board row for every lane that has **never once succeeded**. Two bars, both required: (1) `Error`/`AuthMissing`/`TimedOut` across **N consecutive completed polls** (counts polls via `last_poll_ms` movement, not janitor ticks) — a **flap debounce**, no longer load-bearing since izumi `c2b48c0` persisted the health plane; (2) the verdict is **`Blind`** (`izumi::HealthVerdict`, reached via `SourceHealth::verdict()` — **polled**, and no successful poll on record). A merely-`Degraded` lane (it worked before, it is failing now) files **nothing**: weather belongs in the picker's ambient health footer, and a row per blip is what teaches an operator to stop reading the rows. An **`Unknown`** lane — no poll has ever completed — files nothing either, for the opposite reason: blind is a finding, unknown is the absence of one, and only a finding may gate. The gate is one exhaustive `match` over all four verdicts, so a fifth is a compile error rather than a silent default. `Unconfigured` is deliberately absent from bar 1 — the prescribed config arms 26 sources on purpose and lets each degrade to "needs config", so an unparameterized lane is a *chosen* state, not a surprising one. Observe-only in M0 (`remediable = false`) — re-arming a source is an operator move (token/config), so the janitor surfaces it instead of pretending to fix it. |
