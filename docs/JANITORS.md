...
| **SuggestHealthJanitor** | `src/janitors.rs` | Watches the izumi store's per-source `SourceHealth` for sources in `Error`/`AuthMissing` across **N consecutive completed polls** (counts polls via `last_poll_ms` movement, not janitor ticks). Observe-only in M0 (`remediable = false`) — re-arming a source is an operator move (token/config), so the janitor surfaces it instead of pretending to fix it. |
