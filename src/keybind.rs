...
mod tests {
    ...
    fn all_actions_serializable() {
        let actions = [
            ...
            // Was MISSING until 2026-08-01 — the drift the guard below
    ...
    /// `all_actions_serializable` is a HAND-LISTED array, and its name is a
    /// promise its body cannot keep — adding an `Action` variant does not add
    /// it to that list. It had already drifted when this guard was written
    /// (2026-08-01): 28 of 29 variants listed, `SaveSessionAsPreset` missing.
    /// A test called "all actions" that silently covers all-but-one is worse
    /// than one honestly named for its subset, because a reader budgets trust
    /// against the name.
    ///
    /// So this reads the enum out of the source and holds the list to it.
    ///
    /// **The real fix is one level up and is named, not done here:**
    /// `KindStr` (pleme-kindstr-derive, itself GENERATED by
    /// `tatara-rust-derive::KindRoundTripSpec`) emits `as_str` +
    /// `from_str_kind` but NO exhaustive `ALL`. With
    /// `pub const ALL: &[Self]` the array above becomes `Action::ALL` and the
    /// drift stops being possible rather than being caught — and every
    /// `KindStr` consumer in the fleet inherits it, since a command palette,
    /// a keybinding doc and a discoverability tree are all the same
    /// "enumerate the intents" problem. That is a macro-farm change; this
    /// guard is the honest floor until it lands.
    ///
    /// Tier: **CI-caught**, not unrepresentable.
    #[test]
    fn the_all_actions_test_actually_lists_every_action() {
        ...
        let listed = src
            .split_once("fn all_actions_serializable()")
            .expect("the hand-listed test must exist")
            .1;
        let listed = listed.split_once("];").expect("its array must terminate").0;

        let missing: Vec<&&str> = declared
            .iter()
            .filter(|v| !listed.contains(&format!("Action::{v}")))
            .collect();
        assert!(
            missing.is_empty(),
            "`all_actions_serializable` does not list {} declared \
             Action variant(s): {missing:?}\nAdd them there, or land \
             `Action::ALL` in the KindStr derive so the list is generated \
             and this whole class goes away.",
            missing.len()
        );
    }
